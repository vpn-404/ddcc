...
// const UAs = fs.readFileSync('ua.txt', 'utf-8')
...
let pdata = '';
let pdata_len= 0;
if (VarsDefinetions.MethodRequest == 'POST'){
	if (VarsDefinetions.pdata.indexOf("[rand]") !== -1){
		pdata = VarsDefinetions.pdata.replace(/\[rand\]/g,RandomString(getRandomNumberBetween(5,16)));
	}else{
		pdata = ''+VarsDefinetions.pdata;
	}
	pdata_len = Buffer.byteLength(pdata, 'utf8')
}
...
var aa = getRandomNumberBetween(400, proxies.length-100);
